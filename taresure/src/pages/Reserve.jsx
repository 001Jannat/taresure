import React from 'react';
import Header from '../components/Header';
import NavFooter from '../components/NavFooter';

const Reserve = () => {
  return (
<>

<div id="dev-id">
  <div data-v-b02889ae="" className="pc-class">
    <div data-v-b02889ae="" className="ivu-layout">

      <Header />
      <div data-v-b02889ae="" className="content-wrap">
        <div
          data-v-5f6adc01=""
          data-v-b02889ae=""
          className="defi"
          footernavitems="[object Object],[object Object],[object Object],[object Object],[object Object]"
        >
          {/**/}
          <div
            data-v-5f6adc01=""
            className="mainData ivu-row-flex ivu-row-flex-space-between"
          >
            <div
              data-v-5f6adc01=""
              className="ivu-col ivu-col-span-xs-24 ivu-col-span-sm-24 ivu-col-span-md-14 ivu-col-span-lg-14 ivu-col-span-xl-14"
            />
            <div
              data-v-5f6adc01=""
              className="right__Col ivu-col ivu-col-span-xs-24 ivu-col-span-sm-24 ivu-col-span-md-10 ivu-col-span-lg-10 ivu-col-span-xl-10"
            >
              <div
                data-v-1003fd90=""
                data-v-5f6adc01=""
                className="UserStatistics"
              >
                <div
                  data-v-1003fd90=""
                  className="ivu-row margin-top-20 margin-bottom-20"
                  style={{ marginLeft: "-8px", marginRight: "-8px" }}
                >
                  <div
                    data-v-1003fd90=""
                    className="ivu-col ivu-col-span-xs-24 ivu-col-span-sm-24 ivu-col-span-md-24 ivu-col-span-lg-24 ivu-col-span-xl-24"
                    style={{ paddingLeft: 8, paddingRight: 8 }}
                  >
                    <div
                      data-v-1003fd90=""
                      className="UserStatistics__outSideDiv"
                    >
                      <div
                        data-v-1003fd90=""
                        className="UserStatistics__outSideDiv__bgBox UserStatistics__outSideDiv__blue"
                      >
                        <div
                          data-v-1003fd90=""
                          className="title-grey666-PFR-12 UserStatistics__outSideDiv__level-title"
                        >
                          Today Earnings
                        </div>
                        <div
                          data-v-1003fd90=""
                          className="word-break title-black-SB-18"
                        >
                          0
                        </div>
                      </div>
                      <div
                        data-v-1003fd90=""
                        className="UserStatistics__outSideDiv__bgBox UserStatistics__outSideDiv__green"
                      >
                        <div
                          data-v-1003fd90=""
                          className="title-grey666-PFR-12 UserStatistics__outSideDiv__level-title"
                        >
                          Cumulative Income
                        </div>
                        <div
                          data-v-1003fd90=""
                          className="title-black-SB-18 word-break"
                        >
                          0
                        </div>
                      </div>
                      <div
                        data-v-1003fd90=""
                        className="UserStatistics__outSideDiv__bgBox UserStatistics__outSideDiv__grey"
                      >
                        <div
                          data-v-1003fd90=""
                          className="title-grey666-PFR-12 UserStatistics__outSideDiv__level-title"
                        >
                          Team Benefits
                        </div>
                        <div
                          data-v-1003fd90=""
                          className="title-black-SB-18 word-break"
                        >
                          0
                        </div>
                        {/**/}
                      </div>
                      <div
                        data-v-1003fd90=""
                        className="UserStatistics__outSideDiv__bgBox UserStatistics__outSideDiv__orange"
                      >
                        <div
                          data-v-1003fd90=""
                          className="title-grey666-PFR-12 UserStatistics__outSideDiv__level-title"
                        >
                          Reservation range
                        </div>
                        <div
                          data-v-1003fd90=""
                          className="title-black-SB-18 word-break"
                        >
                          50 ~ 150
                        </div>
                      </div>
                      <div
                        data-v-1003fd90=""
                        className="UserStatistics__outSideDiv__bgBox UserStatistics__outSideDiv__cyan"
                      >
                        <div
                          data-v-1003fd90=""
                          className="title-grey666-PFR-12 UserStatistics__outSideDiv__level-title"
                        >
                          Wallet Balance
                        </div>
                        <div
                          data-v-1003fd90=""
                          className="title-black-SB-18 word-break"
                        >
                          0
                        </div>
                      </div>
                      <div
                        data-v-1003fd90=""
                        className="UserStatistics__outSideDiv__bgBox UserStatistics__outSideDiv__black"
                      >
                        <div
                          data-v-1003fd90=""
                          className="title-grey666-PFR-12 UserStatistics__outSideDiv__level-title"
                        >
                          Balance for Reservation
                        </div>
                        <div
                          data-v-1003fd90=""
                          className="title-black-SB-18 word-break"
                        >
                          0
                        </div>
                      </div>
                    </div>
                  </div>
                </div>
              </div>
              <div
                data-v-5f6adc01=""
                className="wrap-col countdown-area  ivu-row-flex"
              >
                <div
                  data-v-5f6adc01=""
                  className="countdown-area__BottomLayOut ivu-col ivu-col-span-24"
                >
                  <div
                    data-v-5f6adc01=""
                    className="ivu-tabs ivu-tabs-no-animation"
                  >
                    <div className="ivu-tabs-bar">
                      {/**/}
                      <div tabIndex={0} className="ivu-tabs-nav-container">
                        <div
                          className="ivu-tabs-nav-wrap"
                          style={{ position: "relative" }}
                        >
                          <span className="ivu-tabs-nav-prev ivu-tabs-nav-scroll-disabled">
                            <i className="ivu-icon ivu-icon-ios-arrow-back" />
                          </span>
                          <span className="ivu-tabs-nav-next ivu-tabs-nav-scroll-disabled">
                            <i className="ivu-icon ivu-icon-ios-arrow-forward" />
                          </span>
                          <div className="ivu-tabs-nav-scroll overflow-nav-scroll">
                            <div className="ivu-tabs-nav">
                              <div
                                className="ivu-tabs-ink-bar"
                                style={{
                                  visibility: "visible",
                                  width: 86,
                                  left: 0
                                }}
                              />
                              <div
                                draggable="false"
                                className="ivu-tabs-tab ivu-tabs-tab-active ivu-tabs-tab-focused"
                              >
                                {/**/}Today's{/**/}
                              </div>
                              <div draggable="false" className="ivu-tabs-tab">
                                {/**/}Reserve{/**/}
                              </div>
                              <div draggable="false" className="ivu-tabs-tab">
                                {/**/}Collection{/**/}
                              </div>
                            </div>
                          </div>
                          <object
                            tabIndex={-1}
                            type="text/html"
                            aria-hidden="true"
                            data="about:blank"
                            style={{
                              display: "block",
                              position: "absolute",
                              top: 0,
                              left: 0,
                              width: "100%",
                              height: "100%",
                              border: "none",
                              padding: 0,
                              margin: 0,
                              opacity: 0,
                              zIndex: -1000,
                              pointerEvents: "none"
                            }}
                          />
                        </div>
                      </div>
                    </div>
                    <div
                      className="ivu-tabs-content"
                      style={{ transform: "translateX(0%) translateZ(0px)" }}
                    >
                      {/**/}
                      <div
                        data-v-5f6adc01=""
                        className="ivu-tabs-tabpane"
                        style={{ visibility: "visible" }}
                      >
                        <div data-v-5f6adc01="" className="CopyReservation">
                          <div className="list-area">
                            <div
                              className="ivu-row"
                              style={{
                                marginLeft: "-8px",
                                marginRight: "-8px"
                              }}
                            >
                              <div className="noData">
                                <img
                                  src="https://image.treasurenft.xyz/icon/icon_noData_01.svg"
                                  alt="NoData"
                                  loading="lazy"
                                />
                                <p className="title-black-PSB-16 font-weight-700 margin-top-30">
                                  No Data Available
                                </p>
                                <p className="title-black-PSB-16 font-weight-700 margin-bottom-10">
                                  GMT+05:00 2025-05-15 16:41:44
                                </p>
                              </div>
                            </div>
                          </div>
                          {/**/}
                          {/**/}
                        </div>
                      </div>
                      <div
                        data-v-5f6adc01=""
                        className="ivu-tabs-tabpane"
                        style={{ visibility: "hidden", display: "none" }}
                      >
                        <div
                          data-v-5f6adc01=""
                          className="countdown-area__topLayOut"
                        >
                          <div
                            data-v-5f6adc01=""
                            className="ivu-row-flex ivu-row-flex-middle title-grey666-PR-12 countdown-area-row"
                            style={{ marginLeft: "-2px", marginRight: "-2px" }}
                          >
                            <div
                              data-v-5f6adc01=""
                              className="ivu-col ivu-col-span-xs-2 ivu-col-span-md-2"
                              style={{ paddingLeft: 2, paddingRight: 2 }}
                            >
                              <div data-v-7a5d90b0="" data-v-5f6adc01="">
                                <div
                                  data-v-7a5d90b0=""
                                  className="ivu-row-flex ivu-row-flex-middle ivu-row-flex-center"
                                >
                                  <div data-v-7a5d90b0="" className="ivu-col">
                                    <img
                                      data-v-7a5d90b0=""
                                      src="https://image.treasurenft.xyz/icon/icon_question-mark_01.svg"
                                      alt="question-mark"
                                      aria-label="Open Help Modal"
                                      width={24}
                                      height={24}
                                      decoding="async"
                                      className="cursor-pointer img"
                                    />
                                  </div>
                                </div>
                                {/**/}
                              </div>
                            </div>
                            <div
                              data-v-5f6adc01=""
                              className="ivu-col ivu-col-span-xs-10 ivu-col-span-sm-11 ivu-col-span-md-11 ivu-col-span-lg-11 ivu-col-span-xl-11"
                              style={{ paddingLeft: 2, paddingRight: 2 }}
                            >
                              <div
                                data-v-5f6adc01=""
                                className="ivu-select ivu-select-single ivu-select-default"
                              >
                                <div
                                  tabIndex={0}
                                  className="ivu-select-selection"
                                >
                                  <input
                                    type="hidden"
                                    defaultValue="[object Object]"
                                  />{" "}
                                  <div className="">
                                    <span className="ivu-select-prefix">
                                      <span
                                        data-v-5f6adc01=""
                                        className="prefix-coin-img ivu-avatar ivu-avatar-circle ivu-avatar-image ivu-avatar-small"
                                      >
                                        <img src="data:image/png;base64," />
                                      </span>
                                    </span>{" "}
                                    {/**/}{" "}
                                    <span className="ivu-select-head-with-prefix ivu-select-selected-value">
                                      50 - 150
                                    </span>{" "}
                                    {/**/} {/**/}{" "}
                                    <i className="ivu-icon ivu-icon-ios-arrow-down ivu-select-arrow" />
                                  </div>
                                </div>{" "}
                                {/**/}
                              </div>
                            </div>
                            <div
                              data-v-5f6adc01=""
                              className="ivu-col ivu-col-span-xs-12 ivu-col-span-sm-11 ivu-col-span-md-11 ivu-col-span-lg-11 ivu-col-span-xl-11"
                              style={{ paddingLeft: 2, paddingRight: 2 }}
                            >
                              <div
                                data-v-5f6adc01=""
                                className="ivu-select ivu-select-single ivu-select-default"
                              >
                                <div
                                  tabIndex={0}
                                  className="ivu-select-selection"
                                >
                                  <input type="hidden" defaultValue={0} />{" "}
                                  <div className="">
                                    <span className="ivu-select-prefix">
                                      <div
                                        data-v-5f6adc01=""
                                        className="countdown-area__selectIcon"
                                      >
                                        <span
                                          data-v-5f6adc01=""
                                          className="title-green-PR-14"
                                        >
                                          1.8-1.95%
                                        </span>
                                      </div>
                                    </span>{" "}
                                    {/**/}{" "}
                                    <span className="ivu-select-head-with-prefix ivu-select-selected-value">
                                      LV 0
                                    </span>{" "}
                                    {/**/} {/**/}{" "}
                                    <i className="ivu-icon ivu-icon-ios-arrow-down ivu-select-arrow" />
                                  </div>
                                </div>{" "}
                                {/**/}
                              </div>
                            </div>
                          </div>
                        </div>
                        <div
                          data-v-5f6adc01=""
                          className="ivu-row-flex ivu-row-flex-space-around"
                        >
                          <div
                            data-v-5f6adc01=""
                            className="ivu-col ivu-col-span-24"
                          >
                            <button
                              data-v-5f6adc01=""
                              type="button"
                              className="countdown-area__newBtn ivu-btn ivu-btn-success ivu-btn-long"
                              fdprocessedid="s3rt5"
                            >
                              {/**/} {/**/} <span>Confirm</span>
                            </button>
                          </div>
                        </div>
                      </div>
                      <div
                        data-v-5f6adc01=""
                        className="ivu-tabs-tabpane"
                        style={{ visibility: "hidden", display: "none" }}
                      />
                    </div>
                    <div
                      className="ivu-tabs-context-menu"
                      style={{ top: 0, left: 0 }}
                    >
                      <div className="ivu-dropdown">
                        <div className="ivu-dropdown-rel" /> {/**/}
                      </div>
                    </div>
                  </div>
                </div>
              </div>
            </div>
            <div
              data-v-5f6adc01=""
              className="margin-top-15 ivu-col ivu-col-span-24"
              style={{ paddingLeft: 0 }}
            />
          </div>
          <div data-v-5f6adc01="">{/**/}</div>
          <div data-v-adaadefc="" data-v-5f6adc01="">
            {/**/}
          </div>
          {/**/}
          {/**/}
          <div data-v-d9d51de4="" data-v-5f6adc01="">
            {/**/}
            {/**/}
          </div>
          {/**/}
          {/**/}
        </div>
        <img
          data-v-b02889ae=""
          src="https://image.treasurenft.xyz/btn/btn_help_01.png"
          alt="helpIcon"
          loading="lazy"
          className="helpIconStyle"
          style={{}}
        />
      </div>


<NavFooter/>


    </div>
    {/**/}
    <div data-v-d27ce888="" data-v-b02889ae="">
      {/**/}
    </div>
    {/**/}
    {/**/}
    {/**/}
    {/**/}
    {/**/}
    <div data-v-d9d51de4="" data-v-b02889ae="">
      {/**/}
      {/**/}
    </div>
    {/**/}
    {/**/}
    {/**/}
  </div>
</div>



</>
  );
};

export default Reserve; 