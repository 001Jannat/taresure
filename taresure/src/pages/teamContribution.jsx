import React from 'react';

const TeamContribution = () => {
  return (



    <>


<div id="dev-id">
  <div data-v-b02889ae="" className="pc-class">
    <div data-v-b02889ae="" className="ivu-layout">
      <div data-v-b02889ae="" className="content-wrap">
        <div
          data-v-040dafc1=""
          data-v-b02889ae=""
          className="teamContribution"
          footernavitems="[object Object],[object Object],[object Object],[object Object],[object Object]"
        >
          {/**/}
          <div
            data-v-040dafc1=""
            className="bgColor-fff padding-top-10 padding-left-10 padding-right-10"
          >
            <img
              data-v-040dafc1=""
              src="https://image.treasurenft.xyz/NewVer2212/icon/black-arrow-left.png"
              alt="back"
              loading="lazy"
              style={{ width: 30 }}
            />
          </div>
          <div data-v-040dafc1="" className="ivu-row-flex ivu-row-flex-middle">
            <div
              data-v-040dafc1=""
              className="left-side ivu-col ivu-col-span-xs-24 ivu-col-span-sm-24 ivu-col-span-md-24 ivu-col-span-lg-10 ivu-col-span-xl-10"
            >
              <div data-v-040dafc1="" className="left-inside">
                <div
                  data-v-040dafc1=""
                  className="ivu-row-flex ivu-row-flex-middle ivu-row-flex-start padding-left-10 padding-right-10 top-area"
                >
                  <div
                    data-v-040dafc1=""
                    className="select-row ivu-col ivu-col-span-xs-24 ivu-col-span-sm-24 ivu-col-span-md-24 ivu-col-span-lg-24 ivu-col-span-xl-24"
                  >
                    <div data-v-040dafc1="" className="date-block">
                      <div
                        data-v-040dafc1=""
                        className="ivu-date-picker"
                        style={{ width: "100%" }}
                      >
                        <div className="ivu-date-picker-rel">
                          <div className="ivu-input-wrapper ivu-input-wrapper-default ivu-input-type-text ivu-date-picker-editor">
                            {/**/}{" "}
                            <span className="ivu-input-suffix">
                              <i className="ivu-icon ivu-icon-ios-calendar-outline" />
                            </span>{" "}
                            <i className="ivu-icon ivu-icon-ios-loading ivu-load-loop ivu-input-icon ivu-input-icon-validate" />{" "}
                            <input
                              autoComplete="off"
                              spellCheck="false"
                              type="text"
                              placeholder="Select Date"
                              className="ivu-input ivu-input-default ivu-input-with-suffix"
                              fdprocessedid="eajibf"
                            />{" "}
                            {/**/}
                          </div>
                        </div>{" "}
                        <div
                          className="ivu-select-dropdown"
                          style={{ display: "none" }}
                        >
                          <div>
                            <div
                              className="ivu-picker-panel-body-wrapper ivu-date-picker-with-range"
                              steps=""
                            >
                              {/**/}{" "}
                              <div className="ivu-picker-panel-body ivu-picker-panel-body-date">
                                <div className="ivu-picker-panel-content ivu-picker-panel-content-left">
                                  <div className="ivu-date-picker-header">
                                    <span className="ivu-picker-panel-icon-btn ivu-date-picker-prev-btn ivu-date-picker-prev-btn-arrow-double">
                                      <i className="ivu-icon ivu-icon-ios-arrow-back" />
                                    </span>{" "}
                                    <span className="ivu-picker-panel-icon-btn ivu-date-picker-prev-btn ivu-date-picker-prev-btn-arrow">
                                      <i className="ivu-icon ivu-icon-ios-arrow-back" />
                                    </span>{" "}
                                    <span>
                                      <span className="ivu-date-picker-header-label">
                                        May
                                      </span>{" "}
                                      <span className="ivu-date-picker-header-label">
                                        2025
                                      </span>
                                    </span>{" "}
                                    {/**/} {/**/}
                                  </div>{" "}
                                  <div className="ivu-date-picker-cells">
                                    <div className="ivu-date-picker-cells-header">
                                      <span>Sun</span>
                                      <span>Mon</span>
                                      <span>Tue</span>
                                      <span>Wed</span>
                                      <span>Thu</span>
                                      <span>Fri</span>
                                      <span>Sat</span>
                                    </div>{" "}
                                    <span className="ivu-date-picker-cells-cell ivu-date-picker-cells-cell-prev-month">
                                      <em>27</em>
                                    </span>
                                    <span className="ivu-date-picker-cells-cell ivu-date-picker-cells-cell-prev-month">
                                      <em>28</em>
                                    </span>
                                    <span className="ivu-date-picker-cells-cell ivu-date-picker-cells-cell-prev-month">
                                      <em>29</em>
                                    </span>
                                    <span className="ivu-date-picker-cells-cell ivu-date-picker-cells-cell-prev-month">
                                      <em>30</em>
                                    </span>
                                    <span className="ivu-date-picker-cells-cell">
                                      <em>1</em>
                                    </span>
                                    <span className="ivu-date-picker-cells-cell">
                                      <em>2</em>
                                    </span>
                                    <span className="ivu-date-picker-cells-cell">
                                      <em>3</em>
                                    </span>
                                    <span className="ivu-date-picker-cells-cell">
                                      <em>4</em>
                                    </span>
                                    <span className="ivu-date-picker-cells-cell">
                                      <em>5</em>
                                    </span>
                                    <span className="ivu-date-picker-cells-cell">
                                      <em>6</em>
                                    </span>
                                    <span className="ivu-date-picker-cells-cell">
                                      <em>7</em>
                                    </span>
                                    <span className="ivu-date-picker-cells-cell">
                                      <em>8</em>
                                    </span>
                                    <span className="ivu-date-picker-cells-cell">
                                      <em>9</em>
                                    </span>
                                    <span className="ivu-date-picker-cells-cell">
                                      <em>10</em>
                                    </span>
                                    <span className="ivu-date-picker-cells-cell">
                                      <em>11</em>
                                    </span>
                                    <span className="ivu-date-picker-cells-cell">
                                      <em>12</em>
                                    </span>
                                    <span className="ivu-date-picker-cells-cell">
                                      <em>13</em>
                                    </span>
                                    <span className="ivu-date-picker-cells-cell">
                                      <em>14</em>
                                    </span>
                                    <span className="ivu-date-picker-cells-cell ivu-date-picker-cells-cell-today ivu-date-picker-cells-focused">
                                      <em>15</em>
                                    </span>
                                    <span className="ivu-date-picker-cells-cell">
                                      <em>16</em>
                                    </span>
                                    <span className="ivu-date-picker-cells-cell">
                                      <em>17</em>
                                    </span>
                                    <span className="ivu-date-picker-cells-cell">
                                      <em>18</em>
                                    </span>
                                    <span className="ivu-date-picker-cells-cell">
                                      <em>19</em>
                                    </span>
                                    <span className="ivu-date-picker-cells-cell">
                                      <em>20</em>
                                    </span>
                                    <span className="ivu-date-picker-cells-cell">
                                      <em>21</em>
                                    </span>
                                    <span className="ivu-date-picker-cells-cell">
                                      <em>22</em>
                                    </span>
                                    <span className="ivu-date-picker-cells-cell">
                                      <em>23</em>
                                    </span>
                                    <span className="ivu-date-picker-cells-cell">
                                      <em>24</em>
                                    </span>
                                    <span className="ivu-date-picker-cells-cell">
                                      <em>25</em>
                                    </span>
                                    <span className="ivu-date-picker-cells-cell">
                                      <em>26</em>
                                    </span>
                                    <span className="ivu-date-picker-cells-cell">
                                      <em>27</em>
                                    </span>
                                    <span className="ivu-date-picker-cells-cell">
                                      <em>28</em>
                                    </span>
                                    <span className="ivu-date-picker-cells-cell">
                                      <em>29</em>
                                    </span>
                                    <span className="ivu-date-picker-cells-cell">
                                      <em>30</em>
                                    </span>
                                    <span className="ivu-date-picker-cells-cell">
                                      <em>31</em>
                                    </span>
                                    <span className="ivu-date-picker-cells-cell ivu-date-picker-cells-cell-next-month">
                                      <em>1</em>
                                    </span>
                                    <span className="ivu-date-picker-cells-cell ivu-date-picker-cells-cell-next-month">
                                      <em>2</em>
                                    </span>
                                    <span className="ivu-date-picker-cells-cell ivu-date-picker-cells-cell-next-month">
                                      <em>3</em>
                                    </span>
                                    <span className="ivu-date-picker-cells-cell ivu-date-picker-cells-cell-next-month">
                                      <em>4</em>
                                    </span>
                                    <span className="ivu-date-picker-cells-cell ivu-date-picker-cells-cell-next-month">
                                      <em>5</em>
                                    </span>
                                    <span className="ivu-date-picker-cells-cell ivu-date-picker-cells-cell-next-month">
                                      <em>6</em>
                                    </span>
                                    <span className="ivu-date-picker-cells-cell ivu-date-picker-cells-cell-next-month">
                                      <em>7</em>
                                    </span>
                                  </div>
                                </div>{" "}
                                <div className="ivu-picker-panel-content ivu-picker-panel-content-right">
                                  <div className="ivu-date-picker-header">
                                    {/**/} {/**/}{" "}
                                    <span>
                                      <span className="ivu-date-picker-header-label">
                                        June
                                      </span>{" "}
                                      <span className="ivu-date-picker-header-label">
                                        2025
                                      </span>
                                    </span>{" "}
                                    <span className="ivu-picker-panel-icon-btn ivu-date-picker-next-btn ivu-date-picker-next-btn-arrow-double">
                                      <i className="ivu-icon ivu-icon-ios-arrow-forward" />
                                    </span>{" "}
                                    <span className="ivu-picker-panel-icon-btn ivu-date-picker-next-btn ivu-date-picker-next-btn-arrow">
                                      <i className="ivu-icon ivu-icon-ios-arrow-forward" />
                                    </span>
                                  </div>{" "}
                                  <div className="ivu-date-picker-cells">
                                    <div className="ivu-date-picker-cells-header">
                                      <span>Sun</span>
                                      <span>Mon</span>
                                      <span>Tue</span>
                                      <span>Wed</span>
                                      <span>Thu</span>
                                      <span>Fri</span>
                                      <span>Sat</span>
                                    </div>{" "}
                                    <span className="ivu-date-picker-cells-cell">
                                      <em>1</em>
                                    </span>
                                    <span className="ivu-date-picker-cells-cell">
                                      <em>2</em>
                                    </span>
                                    <span className="ivu-date-picker-cells-cell">
                                      <em>3</em>
                                    </span>
                                    <span className="ivu-date-picker-cells-cell">
                                      <em>4</em>
                                    </span>
                                    <span className="ivu-date-picker-cells-cell">
                                      <em>5</em>
                                    </span>
                                    <span className="ivu-date-picker-cells-cell">
                                      <em>6</em>
                                    </span>
                                    <span className="ivu-date-picker-cells-cell">
                                      <em>7</em>
                                    </span>
                                    <span className="ivu-date-picker-cells-cell">
                                      <em>8</em>
                                    </span>
                                    <span className="ivu-date-picker-cells-cell">
                                      <em>9</em>
                                    </span>
                                    <span className="ivu-date-picker-cells-cell">
                                      <em>10</em>
                                    </span>
                                    <span className="ivu-date-picker-cells-cell">
                                      <em>11</em>
                                    </span>
                                    <span className="ivu-date-picker-cells-cell">
                                      <em>12</em>
                                    </span>
                                    <span className="ivu-date-picker-cells-cell">
                                      <em>13</em>
                                    </span>
                                    <span className="ivu-date-picker-cells-cell">
                                      <em>14</em>
                                    </span>
                                    <span className="ivu-date-picker-cells-cell">
                                      <em>15</em>
                                    </span>
                                    <span className="ivu-date-picker-cells-cell">
                                      <em>16</em>
                                    </span>
                                    <span className="ivu-date-picker-cells-cell">
                                      <em>17</em>
                                    </span>
                                    <span className="ivu-date-picker-cells-cell">
                                      <em>18</em>
                                    </span>
                                    <span className="ivu-date-picker-cells-cell">
                                      <em>19</em>
                                    </span>
                                    <span className="ivu-date-picker-cells-cell">
                                      <em>20</em>
                                    </span>
                                    <span className="ivu-date-picker-cells-cell">
                                      <em>21</em>
                                    </span>
                                    <span className="ivu-date-picker-cells-cell">
                                      <em>22</em>
                                    </span>
                                    <span className="ivu-date-picker-cells-cell">
                                      <em>23</em>
                                    </span>
                                    <span className="ivu-date-picker-cells-cell">
                                      <em>24</em>
                                    </span>
                                    <span className="ivu-date-picker-cells-cell">
                                      <em>25</em>
                                    </span>
                                    <span className="ivu-date-picker-cells-cell">
                                      <em>26</em>
                                    </span>
                                    <span className="ivu-date-picker-cells-cell">
                                      <em>27</em>
                                    </span>
                                    <span className="ivu-date-picker-cells-cell">
                                      <em>28</em>
                                    </span>
                                    <span className="ivu-date-picker-cells-cell">
                                      <em>29</em>
                                    </span>
                                    <span className="ivu-date-picker-cells-cell">
                                      <em>30</em>
                                    </span>
                                    <span className="ivu-date-picker-cells-cell ivu-date-picker-cells-cell-next-month">
                                      <em>1</em>
                                    </span>
                                    <span className="ivu-date-picker-cells-cell ivu-date-picker-cells-cell-next-month">
                                      <em>2</em>
                                    </span>
                                    <span className="ivu-date-picker-cells-cell ivu-date-picker-cells-cell-next-month">
                                      <em>3</em>
                                    </span>
                                    <span className="ivu-date-picker-cells-cell ivu-date-picker-cells-cell-next-month">
                                      <em>4</em>
                                    </span>
                                    <span className="ivu-date-picker-cells-cell ivu-date-picker-cells-cell-next-month">
                                      <em>5</em>
                                    </span>
                                    <span className="ivu-date-picker-cells-cell ivu-date-picker-cells-cell-next-month">
                                      <em>6</em>
                                    </span>
                                    <span className="ivu-date-picker-cells-cell ivu-date-picker-cells-cell-next-month">
                                      <em>7</em>
                                    </span>
                                    <span className="ivu-date-picker-cells-cell ivu-date-picker-cells-cell-next-month">
                                      <em>8</em>
                                    </span>
                                    <span className="ivu-date-picker-cells-cell ivu-date-picker-cells-cell-next-month">
                                      <em>9</em>
                                    </span>
                                    <span className="ivu-date-picker-cells-cell ivu-date-picker-cells-cell-next-month">
                                      <em>10</em>
                                    </span>
                                    <span className="ivu-date-picker-cells-cell ivu-date-picker-cells-cell-next-month">
                                      <em>11</em>
                                    </span>
                                    <span className="ivu-date-picker-cells-cell ivu-date-picker-cells-cell-next-month">
                                      <em>12</em>
                                    </span>
                                  </div>
                                </div>{" "}
                                <div
                                  className="ivu-picker-panel-content"
                                  style={{ display: "none" }}
                                >
                                  {/**/}
                                </div>{" "}
                                {/**/}
                              </div>
                            </div>
                          </div>
                        </div>
                      </div>
                    </div>
                  </div>
                  <div
                    data-v-040dafc1=""
                    className="ivu-col ivu-col-span-xs-24 ivu-col-span-sm-24 ivu-col-span-md-24 ivu-col-span-lg-14 ivu-col-span-xl-14"
                  >
                    <div data-v-040dafc1="" className="btnDate-group">
                      <button
                        data-v-040dafc1=""
                        className="btnDate-group-btn-active"
                        fdprocessedid="crsjf9"
                      >
                        All
                      </button>
                      <button
                        data-v-040dafc1=""
                        className="btnDate-group-btn"
                        fdprocessedid="zqhns5"
                      >
                        Today
                      </button>
                      <button
                        data-v-040dafc1=""
                        className="btnDate-group-btn"
                        fdprocessedid="6p8tvs"
                      >
                        Yesterday
                      </button>
                      <button
                        data-v-040dafc1=""
                        className="btnDate-group-btn"
                        fdprocessedid="nlava"
                      >
                        Week
                      </button>
                      <button
                        data-v-040dafc1=""
                        className="btnDate-group-btn"
                        fdprocessedid="kzyzqa"
                      >
                        Month
                      </button>
                    </div>
                  </div>
                </div>
                <div data-v-040dafc1="" className="info-group">
                  <div
                    data-v-040dafc1=""
                    className="ivu-row-flex margin-15 bgColor-fff data-Row"
                  >
                    <div
                      data-v-040dafc1=""
                      className="info-item ivu-col ivu-col-span-12"
                    >
                      <p data-v-040dafc1="" className="title-grey666-PFR-12">
                        All rebates
                      </p>
                      <div data-v-040dafc1="" className="info-group-div">
                        <img
                          data-v-040dafc1=""
                          src="data:image/png;base64,"
                          alt="icon"
                          loading="lazy"
                          className="icon"
                        />
                        <span
                          data-v-040dafc1=""
                          className="title-black-SB-16 font-weight-600 margin-left-5"
                        >
                          0
                        </span>
                      </div>
                    </div>
                    <div
                      data-v-040dafc1=""
                      className="info-item ivu-col ivu-col-span-12"
                    >
                      <p data-v-040dafc1="" className="title-grey666-PFR-12">
                        Rebate A
                      </p>
                      <div data-v-040dafc1="" className="info-group-div">
                        <img
                          data-v-040dafc1=""
                          src="data:image/png;base64,"
                          alt="coin"
                          loading="lazy"
                          className="icon"
                        />
                        <span
                          data-v-040dafc1=""
                          className="title-black-SB-16 font-weight-600 margin-left-5"
                        >
                          0
                        </span>
                      </div>
                    </div>
                    <div
                      data-v-040dafc1=""
                      className="info-item ivu-col ivu-col-span-12"
                    >
                      <p data-v-040dafc1="" className="title-grey666-PFR-12">
                        Rebate B
                      </p>
                      <div data-v-040dafc1="" className="info-group-div">
                        <img
                          data-v-040dafc1=""
                          src="data:image/png;base64,"
                          alt="icon"
                          loading="lazy"
                          className="icon"
                        />
                        <span
                          data-v-040dafc1=""
                          className="title-black-SB-16 font-weight-600 margin-left-5"
                        >
                          0
                        </span>
                      </div>
                    </div>
                    <div
                      data-v-040dafc1=""
                      className="info-item ivu-col ivu-col-span-12"
                    >
                      <p data-v-040dafc1="" className="title-grey666-PFR-12">
                        Rebate C
                      </p>
                      <div data-v-040dafc1="" className="info-group-div">
                        <img
                          data-v-040dafc1=""
                          src="data:image/png;base64,"
                          alt="coin"
                          loading="lazy"
                          className="icon"
                        />
                        <span
                          data-v-040dafc1=""
                          className="title-black-SB-16 font-weight-600 margin-left-5"
                        >
                          0
                        </span>
                      </div>
                    </div>
                  </div>
                </div>
                <div
                  data-v-040dafc1=""
                  className="ivu-tabs margin-15 bgColor-fff"
                >
                  <div className="ivu-tabs-bar">
                    {/**/}{" "}
                    <div tabIndex={0} className="ivu-tabs-nav-container">
                      <div
                        className="ivu-tabs-nav-wrap"
                        style={{ position: "relative" }}
                      >
                        <span className="ivu-tabs-nav-prev ivu-tabs-nav-scroll-disabled">
                          <i className="ivu-icon ivu-icon-ios-arrow-back" />
                        </span>{" "}
                        <span className="ivu-tabs-nav-next ivu-tabs-nav-scroll-disabled">
                          <i className="ivu-icon ivu-icon-ios-arrow-forward" />
                        </span>{" "}
                        <div className="ivu-tabs-nav-scroll">
                          <div className="ivu-tabs-nav">
                            <div
                              className="ivu-tabs-ink-bar ivu-tabs-ink-bar-animated"
                              style={{
                                visibility: "visible",
                                width: 104,
                                transform: "translate3d(0px, 0px, 0px)"
                              }}
                            />{" "}
                            <div
                              draggable="false"
                              className="ivu-tabs-tab ivu-tabs-tab-active ivu-tabs-tab-focused"
                            >
                              {/**/} AMember {/**/}
                            </div>
                            <div draggable="false" className="ivu-tabs-tab">
                              {/**/} BMember {/**/}
                            </div>
                            <div draggable="false" className="ivu-tabs-tab">
                              {/**/} CMember {/**/}
                            </div>
                          </div>
                        </div>
                        <object
                          tabIndex={-1}
                          type="text/html"
                          data="about:blank"
                          style={{
                            display: "block",
                            position: "absolute",
                            top: 0,
                            left: 0,
                            width: "100%",
                            height: "100%",
                            border: "none",
                            padding: 0,
                            margin: 0,
                            opacity: 0,
                            zIndex: -1000,
                            pointerEvents: "none"
                          }}
                        />
                      </div>
                    </div>
                  </div>{" "}
                  <div
                    className="ivu-tabs-content ivu-tabs-content-animated"
                    style={{ transform: "translateX(0%) translateZ(0px)" }}
                  >
                    <div
                      data-v-040dafc1=""
                      className="margin-bottom-15 ivu-tabs-tabpane"
                      style={{ visibility: "visible" }}
                    >
                      <div
                        data-v-040dafc1=""
                        className="margin-left-20 margin-right-20 ivu-row-flex"
                      >
                        <div
                          data-v-040dafc1=""
                          className="ivu-col ivu-col-span-16"
                        >
                          <p
                            data-v-040dafc1=""
                            className="title-black-W15-16 font-weight-600"
                          >
                            AMember
                          </p>
                        </div>
                        <div
                          data-v-040dafc1=""
                          className="ivu-col ivu-col-span-8"
                        >
                          <p
                            data-v-040dafc1=""
                            className="title-black-W15-16 font-weight-600"
                          >
                            Rebate
                          </p>
                        </div>
                      </div>
                    </div>
                    <div
                      data-v-040dafc1=""
                      className="margin-bottom-15 ivu-tabs-tabpane"
                      style={{ visibility: "hidden" }}
                    >
                      <div
                        data-v-040dafc1=""
                        className="margin-left-20 margin-right-20 ivu-row-flex"
                      >
                        <div
                          data-v-040dafc1=""
                          className="ivu-col ivu-col-span-16"
                        >
                          <p
                            data-v-040dafc1=""
                            className="title-black-W15-16 font-weight-600"
                          >
                            BMember
                          </p>
                        </div>
                        <div
                          data-v-040dafc1=""
                          className="ivu-col ivu-col-span-8"
                        >
                          <p
                            data-v-040dafc1=""
                            className="title-black-W15-16 font-weight-600"
                          >
                            Rebate
                          </p>
                        </div>
                      </div>
                    </div>
                    <div
                      data-v-040dafc1=""
                      className="margin-bottom-15 ivu-tabs-tabpane"
                      style={{ visibility: "hidden" }}
                    >
                      <div
                        data-v-040dafc1=""
                        className="margin-left-20 margin-right-20 ivu-row-flex"
                      >
                        <div
                          data-v-040dafc1=""
                          className="ivu-col ivu-col-span-16"
                        >
                          <p
                            data-v-040dafc1=""
                            className="title-black-W15-16 font-weight-600"
                          >
                            CMember
                          </p>
                        </div>
                        <div
                          data-v-040dafc1=""
                          className="ivu-col ivu-col-span-8"
                        >
                          <p
                            data-v-040dafc1=""
                            className="title-black-W15-16 font-weight-600"
                          >
                            Rebate
                          </p>
                        </div>
                      </div>
                    </div>
                  </div>{" "}
                  <div
                    className="ivu-tabs-context-menu"
                    style={{ top: 0, left: 0 }}
                  >
                    <div className="ivu-dropdown">
                      <div className="ivu-dropdown-rel" /> {/**/}
                    </div>
                  </div>
                </div>
              </div>
            </div>
            <div
              data-v-040dafc1=""
              className="right-side ivu-col ivu-col-span-xs-0 ivu-col-span-sm-0 ivu-col-span-md-0 ivu-col-span-lg-14 ivu-col-span-xl-14"
            />
          </div>
        </div>
        <img
          data-v-b02889ae=""
          src="https://image.treasurenft.xyz/btn/btn_help_01.png"
          alt="helpIcon"
          loading="lazy"
          className="helpIconStyle"
          style={{}}
        />
      </div>
    </div>
    {/**/}
    <div data-v-d27ce888="" data-v-b02889ae="">
      {/**/}
    </div>
    {/**/}
    {/**/}
    {/**/}
    {/**/}
    {/**/}
    <div data-v-d9d51de4="" data-v-b02889ae="">
      {/**/}
      {/**/}
    </div>
    {/**/}
    {/**/}
    {/**/}
  </div>
</div>


    </>
  );
};

export default TeamContribution; 