import Header from '../components/Header';
import React, { useState } from 'react';
import { useNavigate } from 'react-router-dom';
import '../styles/Stack.css';

const Collection = () => {



  const navigate = useNavigate();
  const [stakezone, setStakezone] = useState('Exclusive');
  const [tabnavs, settabnavs] = useState('earn');
   
  return (
    <>
    
<div id="dev-id">
  <div data-v-b02889ae="" className="pc-class">
    <div data-v-b02889ae="" className="ivu-layout">
      <div data-v-b02889ae="" className="content-wrap">
        <div
          data-v-0113e2d8=""
          data-v-b02889ae=""
          className="collection"
          footernavitems="[object Object],[object Object],[object Object],[object Object],[object Object]"
        >
          {/**/}
          <div data-v-0113e2d8="" className="collection-main">
            <div
              data-v-0113e2d8=""
              className="tab-area ivu-tabs ivu-tabs-no-animation"
            >
              <div className="ivu-tabs-bar">
                {/**/}
                <div tabIndex={0} className="ivu-tabs-nav-container">
                  <div
                    className="ivu-tabs-nav-wrap"
                    style={{ position: "relative" }}
                  >
                    <span className="ivu-tabs-nav-prev ivu-tabs-nav-scroll-disabled">
                      <i className="ivu-icon ivu-icon-ios-arrow-back" />
                    </span>
                    <span className="ivu-tabs-nav-next ivu-tabs-nav-scroll-disabled">
                      <i className="ivu-icon ivu-icon-ios-arrow-forward" />
                    </span>
                    <div className="ivu-tabs-nav-scroll overflow-nav-scroll">
                      <div className="ivu-tabs-nav">
                        <div
                          className="ivu-tabs-ink-bar"
                           style={{
    visibility: 'visible',
    width: 
      tabnavs === 'earn' ? '65px' :
      tabnavs === 'record' ? '84px' :
      tabnavs === 'Art' ? '55px' :
      tabnavs === 'Collectibles' ? '118px' : '0px',
    left:
      tabnavs === 'earn' ? '0px' :
      tabnavs === 'record' ? '77px' :
      tabnavs === 'Art' ? '214px' :
      tabnavs === 'Collectibles' ? '282px' : '0px'
  }}
                        />
                        <div
                          draggable="false"
                          
                           className={`ivu-tabs-tab  ${ tabnavs === 'earn' ? 'ivu-tabs-tab-active ivu-tabs-tab-focused' : '' }   `}
                           onClick={() => settabnavs('earn')}
                        >
                          {/**/}Earn{/**/}
                        </div>
                        <div draggable="false"  className={`ivu-tabs-tab  ${ tabnavs === 'record' ? 'ivu-tabs-tab-active ivu-tabs-tab-focused' : '' }   `} 
                         onClick={() => settabnavs('record')}>
                          {/**/}Record{/**/}
                        </div>
                      </div>
                    </div>
                    <object
                      tabIndex={-1}
                      type="text/html"
                      aria-hidden="true"
                      data="about:blank"
                      style={{
                        display: "block",
                        position: "absolute",
                        top: 0,
                        left: 0,
                        width: "100%",
                        height: "100%",
                        border: "none",
                        padding: 0,
                        margin: 0,
                        opacity: 0,
                        zIndex: -1000,
                        pointerEvents: "none"
                      }}
                    />
                  </div>
                </div>
              </div>
              <div
                className="ivu-tabs-content"
                style={{ transform: "translateX(0%) translateZ(0px)" }}
              >
                <div
                  data-v-0113e2d8=""
                  className="ivu-tabs-tabpane"
 style={{
  display: tabnavs === 'earn' ? 'block' : 'none',
  visibility: tabnavs === 'earn' ? 'visible' : 'hidden'
}}
                >
                  <div
                    data-v-0113e2d8=""
                    className="list-area ivu-row"
                    style={{ marginLeft: "-6px", marginRight: "-6px" }}
                  >
                    <div
                      data-v-0113e2d8=""
                      className="ivu-col card-gap-y ivu-col-span-xs-24 ivu-col-span-sm-12 ivu-col-span-lg-8 ivu-col-span-xxl-6"
                      style={{ paddingLeft: 6, paddingRight: 6 }}
                    >
                      <div data-v-4214013e="" className="finance-card">
                        <div
                          data-v-0113e2d8=""
                          className="ivu-row-flex ivu-row-flex-middle ivu-row-flex-space-between margin-bottom-10"
                          data-v-4214013e=""
                        >
                          <div
                            data-v-0113e2d8=""
                            className="ivu-col"
                            data-v-4214013e=""
                          >
                            <span
                              data-v-4214013e=""
                              className="finance-card__field"
                            >
                              Level：
                            </span>
                            <span
                              data-v-4214013e=""
                              className="finance-card__value color-green"
                            >
                              Lv1 ~ Lv6
                            </span>
                          </div>
                          <div
                            data-v-0113e2d8=""
                            className="ivu-col"
                            data-v-4214013e=""
                          >
                            <span
                              data-v-4214013e=""
                              className="finance-card__day"
                            >
                              10day
                            </span>
                          </div>
                        </div>
                        <div
                          data-v-0113e2d8=""
                          className="ivu-row-flex margin-bottom-10"
                          data-v-4214013e=""
                        >
                          <div
                            data-v-0113e2d8=""
                            className="ivu-col"
                            data-v-4214013e=""
                            style={{ flex: "1 1 auto" }}
                          >
                            <div
                              data-v-4214013e=""
                              className="finance-card__img-box"
                            >
                              <img
                                data-v-4214013e=""
                                src="https://image.treasurenft.xyz/PC/img/img_pledge_01_lv01.png"
                                alt="finance-level-img"
                                height={207}
                                width={309}
                                className="finance-card__img"
                              />
                              <div
                                data-v-4214013e=""
                                className="finance-card__wedge"
                                style={{
                                  backgroundImage:
                                    'url("https://image.treasurenft.xyz/PC/img/img_wedge_02.svg")'
                                }}
                              >
                                <div
                                  data-v-0113e2d8=""
                                  className="ivu-row-flex ivu-row-flex-middle"
                                  data-v-4214013e=""
                                  style={{
                                    marginLeft: "-3.5px",
                                    marginRight: "-3.5px",
                                    height: "100%"
                                  }}
                                >
                                  <div
                                    data-v-0113e2d8=""
                                    className="ivu-col margin-left-45"
                                    data-v-4214013e=""
                                    style={{
                                      paddingLeft: "3.5px",
                                      paddingRight: "3.5px"
                                    }}
                                  >
                                    <img
                                      data-v-4214013e=""
                                      width={24}
                                      height={24}
                                      src="https://image.treasurenft.xyz/PC/img/icon_curve_up_01.svg"
                                      alt="curve up icon"
                                      className="finance-card__icon"
                                    />
                                  </div>
                                  <div
                                    data-v-0113e2d8=""
                                    className="ivu-col"
                                    data-v-4214013e=""
                                    style={{
                                      paddingLeft: "3.5px",
                                      paddingRight: "3.5px"
                                    }}
                                  >
                                    <span
                                      data-v-4214013e=""
                                      className="finance-card__wedge-text"
                                    >
                                      1.3%
                                    </span>
                                  </div>
                                </div>
                              </div>
                            </div>
                          </div>
                        </div>
                        <div
                          data-v-0113e2d8=""
                          className="ivu-row-flex ivu-row-flex-middle ivu-row-flex-space-between"
                          data-v-4214013e=""
                          style={{ marginLeft: "-8px", marginRight: "-8px" }}
                        >
                          <div
                            data-v-0113e2d8=""
                            className="ivu-col"
                            data-v-4214013e=""
                            style={{
                              flex: "1 1 auto",
                              paddingLeft: 8,
                              paddingRight: 8
                            }}
                          >
                            <div
                              data-v-0113e2d8=""
                              className="ivu-row margin-bottom-10"
                              data-v-4214013e=""
                            >
                              <div
                                data-v-0113e2d8=""
                                className="ivu-col"
                                data-v-4214013e=""
                              >
                                <h4
                                  data-v-4214013e=""
                                  className="finance-card__title"
                                >
                                  YieldCanvas
                                </h4>
                              </div>
                            </div>
                            <div
                              data-v-0113e2d8=""
                              className="ivu-row-flex ivu-row-flex-middle ivu-row-flex-space-between"
                              data-v-4214013e=""
                            >
                              <div
                                data-v-0113e2d8=""
                                className="ivu-col"
                                data-v-4214013e=""
                              >
                                <span
                                  data-v-4214013e=""
                                  className="finance-card__field"
                                >
                                  Amount
                                </span>
                              </div>
                              <div
                                data-v-0113e2d8=""
                                className="ivu-col"
                                data-v-4214013e=""
                              >
                                <div data-v-4214013e="" className="setMiddle">
                                  <img
                                    data-v-4214013e=""
                                    src="data:image/png;base64,"
                                    alt="coin-icon"
                                    width={20}
                                    height={20}
                                    className="margin-right-5"
                                  />
                                  <span
                                    data-v-4214013e=""
                                    className="finance-card__value color-green"
                                  >
                                    100-2000
                                  </span>
                                </div>
                              </div>
                            </div>
                            <div
                              data-v-0113e2d8=""
                              className="ivu-row-flex ivu-row-flex-middle ivu-row-flex-space-between"
                              data-v-4214013e=""
                            >
                              <div
                                data-v-0113e2d8=""
                                className="ivu-col"
                                data-v-4214013e=""
                              >
                                <span
                                  data-v-4214013e=""
                                  className="finance-card__field"
                                >
                                  Daily income
                                </span>
                              </div>
                              <div
                                data-v-0113e2d8=""
                                className="ivu-col"
                                data-v-4214013e=""
                              >
                                <span
                                  data-v-4214013e=""
                                  className="finance-card__value"
                                >
                                  1.3%
                                </span>
                              </div>
                            </div>
                            <div
                              data-v-0113e2d8=""
                              className="ivu-row-flex ivu-row-flex-middle ivu-row-flex-space-between"
                              data-v-4214013e=""
                            >
                              <div
                                data-v-0113e2d8=""
                                className="ivu-col"
                                data-v-4214013e=""
                              >
                                <span
                                  data-v-4214013e=""
                                  className="finance-card__field"
                                >
                                  Fees
                                </span>
                              </div>
                              <div
                                data-v-0113e2d8=""
                                className="ivu-col"
                                data-v-4214013e=""
                              >
                                <span
                                  data-v-4214013e=""
                                  className="finance-card__value"
                                >
                                  0%
                                </span>
                              </div>
                            </div>
                          </div>
                        </div>
                        <div
                          data-v-0113e2d8=""
                          className="ivu-row"
                          data-v-4214013e=""
                        >
                          <div
                            data-v-0113e2d8=""
                            className="ivu-col"
                            data-v-4214013e=""
                          >
                            <hr
                              data-v-4214013e=""
                              className="finance-card__divider"
                            />
                          </div>
                        </div>
                        <div
                          data-v-0113e2d8=""
                          className="ivu-row"
                          data-v-4214013e=""
                        >
                          <div
                            data-v-0113e2d8=""
                            className="ivu-col"
                            data-v-4214013e=""
                          >
                            {/**/}
                            <div
                              data-v-0113e2d8=""
                              className="ivu-row"
                              data-v-4214013e=""
                            >
                              <div
                                data-v-0113e2d8=""
                                className="ivu-col"
                                data-v-4214013e=""
                              >
                                <button
                                  data-v-0113e2d8=""
                                  type="button"
                                  className="ivu-btn ivu-btn-success ivu-btn-long"
                                  data-v-4214013e=""
                                  fdprocessedid="bwbuhe"
                                >
                                  {/**/} {/**/} <span>Details</span>
                                </button>
                              </div>
                            </div>
                          </div>
                        </div>
                      </div>
                    </div>
                    <div
                      data-v-0113e2d8=""
                      className="ivu-col card-gap-y ivu-col-span-xs-24 ivu-col-span-sm-12 ivu-col-span-lg-8 ivu-col-span-xxl-6"
                      style={{ paddingLeft: 6, paddingRight: 6 }}
                    >
                      <div data-v-4214013e="" className="finance-card">
                        <div
                          data-v-0113e2d8=""
                          className="ivu-row-flex ivu-row-flex-middle ivu-row-flex-space-between margin-bottom-10"
                          data-v-4214013e=""
                        >
                          <div
                            data-v-0113e2d8=""
                            className="ivu-col"
                            data-v-4214013e=""
                          >
                            <span
                              data-v-4214013e=""
                              className="finance-card__field"
                            >
                              Level：
                            </span>
                            <span
                              data-v-4214013e=""
                              className="finance-card__value color-green"
                            >
                              Lv1 ~ Lv6
                            </span>
                          </div>
                          <div
                            data-v-0113e2d8=""
                            className="ivu-col"
                            data-v-4214013e=""
                          >
                            <span
                              data-v-4214013e=""
                              className="finance-card__day"
                            >
                              30day
                            </span>
                          </div>
                        </div>
                        <div
                          data-v-0113e2d8=""
                          className="ivu-row-flex margin-bottom-10"
                          data-v-4214013e=""
                        >
                          <div
                            data-v-0113e2d8=""
                            className="ivu-col"
                            data-v-4214013e=""
                            style={{ flex: "1 1 auto" }}
                          >
                            <div
                              data-v-4214013e=""
                              className="finance-card__img-box"
                            >
                              <img
                                data-v-4214013e=""
                                src="https://image.treasurenft.xyz/PC/img/img_pledge_01_lv02.png"
                                alt="finance-level-img"
                                height={207}
                                width={309}
                                className="finance-card__img"
                              />
                              <div
                                data-v-4214013e=""
                                className="finance-card__wedge"
                                style={{
                                  backgroundImage:
                                    'url("https://image.treasurenft.xyz/PC/img/img_wedge_02.svg")'
                                }}
                              >
                                <div
                                  data-v-0113e2d8=""
                                  className="ivu-row-flex ivu-row-flex-middle"
                                  data-v-4214013e=""
                                  style={{
                                    marginLeft: "-3.5px",
                                    marginRight: "-3.5px",
                                    height: "100%"
                                  }}
                                >
                                  <div
                                    data-v-0113e2d8=""
                                    className="ivu-col margin-left-45"
                                    data-v-4214013e=""
                                    style={{
                                      paddingLeft: "3.5px",
                                      paddingRight: "3.5px"
                                    }}
                                  >
                                    <img
                                      data-v-4214013e=""
                                      width={24}
                                      height={24}
                                      src="https://image.treasurenft.xyz/PC/img/icon_curve_up_01.svg"
                                      alt="curve up icon"
                                      className="finance-card__icon"
                                    />
                                  </div>
                                  <div
                                    data-v-0113e2d8=""
                                    className="ivu-col"
                                    data-v-4214013e=""
                                    style={{
                                      paddingLeft: "3.5px",
                                      paddingRight: "3.5px"
                                    }}
                                  >
                                    <span
                                      data-v-4214013e=""
                                      className="finance-card__wedge-text"
                                    >
                                      1.8%
                                    </span>
                                  </div>
                                </div>
                              </div>
                            </div>
                          </div>
                        </div>
                        <div
                          data-v-0113e2d8=""
                          className="ivu-row-flex ivu-row-flex-middle ivu-row-flex-space-between"
                          data-v-4214013e=""
                          style={{ marginLeft: "-8px", marginRight: "-8px" }}
                        >
                          <div
                            data-v-0113e2d8=""
                            className="ivu-col"
                            data-v-4214013e=""
                            style={{
                              flex: "1 1 auto",
                              paddingLeft: 8,
                              paddingRight: 8
                            }}
                          >
                            <div
                              data-v-0113e2d8=""
                              className="ivu-row margin-bottom-10"
                              data-v-4214013e=""
                            >
                              <div
                                data-v-0113e2d8=""
                                className="ivu-col"
                                data-v-4214013e=""
                              >
                                <h4
                                  data-v-4214013e=""
                                  className="finance-card__title"
                                >
                                  ArtisticAlpha
                                </h4>
                              </div>
                            </div>
                            <div
                              data-v-0113e2d8=""
                              className="ivu-row-flex ivu-row-flex-middle ivu-row-flex-space-between"
                              data-v-4214013e=""
                            >
                              <div
                                data-v-0113e2d8=""
                                className="ivu-col"
                                data-v-4214013e=""
                              >
                                <span
                                  data-v-4214013e=""
                                  className="finance-card__field"
                                >
                                  Amount
                                </span>
                              </div>
                              <div
                                data-v-0113e2d8=""
                                className="ivu-col"
                                data-v-4214013e=""
                              >
                                <div data-v-4214013e="" className="setMiddle">
                                  <img
                                    data-v-4214013e=""
                                    src="data:image/png;base64,"
                                    alt="coin-icon"
                                    width={20}
                                    height={20}
                                    className="margin-right-5"
                                  />
                                  <span
                                    data-v-4214013e=""
                                    className="finance-card__value color-green"
                                  >
                                    100-5000
                                  </span>
                                </div>
                              </div>
                            </div>
                            <div
                              data-v-0113e2d8=""
                              className="ivu-row-flex ivu-row-flex-middle ivu-row-flex-space-between"
                              data-v-4214013e=""
                            >
                              <div
                                data-v-0113e2d8=""
                                className="ivu-col"
                                data-v-4214013e=""
                              >
                                <span
                                  data-v-4214013e=""
                                  className="finance-card__field"
                                >
                                  Daily income
                                </span>
                              </div>
                              <div
                                data-v-0113e2d8=""
                                className="ivu-col"
                                data-v-4214013e=""
                              >
                                <span
                                  data-v-4214013e=""
                                  className="finance-card__value"
                                >
                                  1.8%
                                </span>
                              </div>
                            </div>
                            <div
                              data-v-0113e2d8=""
                              className="ivu-row-flex ivu-row-flex-middle ivu-row-flex-space-between"
                              data-v-4214013e=""
                            >
                              <div
                                data-v-0113e2d8=""
                                className="ivu-col"
                                data-v-4214013e=""
                              >
                                <span
                                  data-v-4214013e=""
                                  className="finance-card__field"
                                >
                                  Fees
                                </span>
                              </div>
                              <div
                                data-v-0113e2d8=""
                                className="ivu-col"
                                data-v-4214013e=""
                              >
                                <span
                                  data-v-4214013e=""
                                  className="finance-card__value"
                                >
                                  0%
                                </span>
                              </div>
                            </div>
                          </div>
                        </div>
                        <div
                          data-v-0113e2d8=""
                          className="ivu-row"
                          data-v-4214013e=""
                        >
                          <div
                            data-v-0113e2d8=""
                            className="ivu-col"
                            data-v-4214013e=""
                          >
                            <hr
                              data-v-4214013e=""
                              className="finance-card__divider"
                            />
                          </div>
                        </div>
                        <div
                          data-v-0113e2d8=""
                          className="ivu-row"
                          data-v-4214013e=""
                        >
                          <div
                            data-v-0113e2d8=""
                            className="ivu-col"
                            data-v-4214013e=""
                          >
                            {/**/}
                            <div
                              data-v-0113e2d8=""
                              className="ivu-row"
                              data-v-4214013e=""
                            >
                              <div
                                data-v-0113e2d8=""
                                className="ivu-col"
                                data-v-4214013e=""
                              >
                                <button
                                  data-v-0113e2d8=""
                                  type="button"
                                  className="ivu-btn ivu-btn-success ivu-btn-long"
                                  data-v-4214013e=""
                                  fdprocessedid="zl6ftm"
                                >
                                  {/**/} {/**/} <span>Details</span>
                                </button>
                              </div>
                            </div>
                          </div>
                        </div>
                      </div>
                    </div>
                    <div
                      data-v-0113e2d8=""
                      className="ivu-col card-gap-y ivu-col-span-xs-24 ivu-col-span-sm-12 ivu-col-span-lg-8 ivu-col-span-xxl-6"
                      style={{ paddingLeft: 6, paddingRight: 6 }}
                    >
                      <div data-v-4214013e="" className="finance-card">
                        <div
                          data-v-0113e2d8=""
                          className="ivu-row-flex ivu-row-flex-middle ivu-row-flex-space-between margin-bottom-10"
                          data-v-4214013e=""
                        >
                          <div
                            data-v-0113e2d8=""
                            className="ivu-col"
                            data-v-4214013e=""
                          >
                            <span
                              data-v-4214013e=""
                              className="finance-card__field"
                            >
                              Level：
                            </span>
                            <span
                              data-v-4214013e=""
                              className="finance-card__value color-green"
                            >
                              Lv1 ~ Lv6
                            </span>
                          </div>
                          <div
                            data-v-0113e2d8=""
                            className="ivu-col"
                            data-v-4214013e=""
                          >
                            <span
                              data-v-4214013e=""
                              className="finance-card__day"
                            >
                              60day
                            </span>
                          </div>
                        </div>
                        <div
                          data-v-0113e2d8=""
                          className="ivu-row-flex margin-bottom-10"
                          data-v-4214013e=""
                        >
                          <div
                            data-v-0113e2d8=""
                            className="ivu-col"
                            data-v-4214013e=""
                            style={{ flex: "1 1 auto" }}
                          >
                            <div
                              data-v-4214013e=""
                              className="finance-card__img-box"
                            >
                              <img
                                data-v-4214013e=""
                                src="https://image.treasurenft.xyz/PC/img/img_pledge_01_lv03.png"
                                alt="finance-level-img"
                                height={207}
                                width={309}
                                className="finance-card__img"
                              />
                              <div
                                data-v-4214013e=""
                                className="finance-card__wedge"
                                style={{
                                  backgroundImage:
                                    'url("https://image.treasurenft.xyz/PC/img/img_wedge_02.svg")'
                                }}
                              >
                                <div
                                  data-v-0113e2d8=""
                                  className="ivu-row-flex ivu-row-flex-middle"
                                  data-v-4214013e=""
                                  style={{
                                    marginLeft: "-3.5px",
                                    marginRight: "-3.5px",
                                    height: "100%"
                                  }}
                                >
                                  <div
                                    data-v-0113e2d8=""
                                    className="ivu-col margin-left-45"
                                    data-v-4214013e=""
                                    style={{
                                      paddingLeft: "3.5px",
                                      paddingRight: "3.5px"
                                    }}
                                  >
                                    <img
                                      data-v-4214013e=""
                                      width={24}
                                      height={24}
                                      src="https://image.treasurenft.xyz/PC/img/icon_curve_up_01.svg"
                                      alt="curve up icon"
                                      className="finance-card__icon"
                                    />
                                  </div>
                                  <div
                                    data-v-0113e2d8=""
                                    className="ivu-col"
                                    data-v-4214013e=""
                                    style={{
                                      paddingLeft: "3.5px",
                                      paddingRight: "3.5px"
                                    }}
                                  >
                                    <span
                                      data-v-4214013e=""
                                      className="finance-card__wedge-text"
                                    >
                                      2.5%
                                    </span>
                                  </div>
                                </div>
                              </div>
                            </div>
                          </div>
                        </div>
                        <div
                          data-v-0113e2d8=""
                          className="ivu-row-flex ivu-row-flex-middle ivu-row-flex-space-between"
                          data-v-4214013e=""
                          style={{ marginLeft: "-8px", marginRight: "-8px" }}
                        >
                          <div
                            data-v-0113e2d8=""
                            className="ivu-col"
                            data-v-4214013e=""
                            style={{
                              flex: "1 1 auto",
                              paddingLeft: 8,
                              paddingRight: 8
                            }}
                          >
                            <div
                              data-v-0113e2d8=""
                              className="ivu-row margin-bottom-10"
                              data-v-4214013e=""
                            >
                              <div
                                data-v-0113e2d8=""
                                className="ivu-col"
                                data-v-4214013e=""
                              >
                                <h4
                                  data-v-4214013e=""
                                  className="finance-card__title"
                                >
                                  WealthVista
                                </h4>
                              </div>
                            </div>
                            <div
                              data-v-0113e2d8=""
                              className="ivu-row-flex ivu-row-flex-middle ivu-row-flex-space-between"
                              data-v-4214013e=""
                            >
                              <div
                                data-v-0113e2d8=""
                                className="ivu-col"
                                data-v-4214013e=""
                              >
                                <span
                                  data-v-4214013e=""
                                  className="finance-card__field"
                                >
                                  Amount
                                </span>
                              </div>
                              <div
                                data-v-0113e2d8=""
                                className="ivu-col"
                                data-v-4214013e=""
                              >
                                <div data-v-4214013e="" className="setMiddle">
                                  <img
                                    data-v-4214013e=""
                                    src="data:image/png;base64,"
                                    alt="coin-icon"
                                    width={20}
                                    height={20}
                                    className="margin-right-5"
                                  />
                                  <span
                                    data-v-4214013e=""
                                    className="finance-card__value color-green"
                                  >
                                    100-10000
                                  </span>
                                </div>
                              </div>
                            </div>
                            <div
                              data-v-0113e2d8=""
                              className="ivu-row-flex ivu-row-flex-middle ivu-row-flex-space-between"
                              data-v-4214013e=""
                            >
                              <div
                                data-v-0113e2d8=""
                                className="ivu-col"
                                data-v-4214013e=""
                              >
                                <span
                                  data-v-4214013e=""
                                  className="finance-card__field"
                                >
                                  Daily income
                                </span>
                              </div>
                              <div
                                data-v-0113e2d8=""
                                className="ivu-col"
                                data-v-4214013e=""
                              >
                                <span
                                  data-v-4214013e=""
                                  className="finance-card__value"
                                >
                                  2.5%
                                </span>
                              </div>
                            </div>
                            <div
                              data-v-0113e2d8=""
                              className="ivu-row-flex ivu-row-flex-middle ivu-row-flex-space-between"
                              data-v-4214013e=""
                            >
                              <div
                                data-v-0113e2d8=""
                                className="ivu-col"
                                data-v-4214013e=""
                              >
                                <span
                                  data-v-4214013e=""
                                  className="finance-card__field"
                                >
                                  Fees
                                </span>
                              </div>
                              <div
                                data-v-0113e2d8=""
                                className="ivu-col"
                                data-v-4214013e=""
                              >
                                <span
                                  data-v-4214013e=""
                                  className="finance-card__value"
                                >
                                  0%
                                </span>
                              </div>
                            </div>
                          </div>
                        </div>
                        <div
                          data-v-0113e2d8=""
                          className="ivu-row"
                          data-v-4214013e=""
                        >
                          <div
                            data-v-0113e2d8=""
                            className="ivu-col"
                            data-v-4214013e=""
                          >
                            <hr
                              data-v-4214013e=""
                              className="finance-card__divider"
                            />
                          </div>
                        </div>
                        <div
                          data-v-0113e2d8=""
                          className="ivu-row"
                          data-v-4214013e=""
                        >
                          <div
                            data-v-0113e2d8=""
                            className="ivu-col"
                            data-v-4214013e=""
                          >
                            {/**/}
                            <div
                              data-v-0113e2d8=""
                              className="ivu-row"
                              data-v-4214013e=""
                            >
                              <div
                                data-v-0113e2d8=""
                                className="ivu-col"
                                data-v-4214013e=""
                              >
                                <button
                                  data-v-0113e2d8=""
                                  type="button"
                                  className="ivu-btn ivu-btn-success ivu-btn-long"
                                  data-v-4214013e=""
                                  fdprocessedid="b6kcsp"
                                >
                                  {/**/} {/**/} <span>Details</span>
                                </button>
                              </div>
                            </div>
                          </div>
                        </div>
                      </div>
                    </div>
                    <div
                      data-v-0113e2d8=""
                      className="ivu-col card-gap-y ivu-col-span-xs-24 ivu-col-span-sm-12 ivu-col-span-lg-8 ivu-col-span-xxl-6"
                      style={{ paddingLeft: 6, paddingRight: 6 }}
                    >
                      <div data-v-4214013e="" className="finance-card">
                        <div
                          data-v-0113e2d8=""
                          className="ivu-row-flex ivu-row-flex-middle ivu-row-flex-space-between margin-bottom-10"
                          data-v-4214013e=""
                        >
                          <div
                            data-v-0113e2d8=""
                            className="ivu-col"
                            data-v-4214013e=""
                          >
                            <span
                              data-v-4214013e=""
                              className="finance-card__field"
                            >
                              Level：
                            </span>
                            <span
                              data-v-4214013e=""
                              className="finance-card__value color-green"
                            >
                              Lv1 ~ Lv6
                            </span>
                          </div>
                          <div
                            data-v-0113e2d8=""
                            className="ivu-col"
                            data-v-4214013e=""
                          >
                            <span
                              data-v-4214013e=""
                              className="finance-card__day"
                            >
                              90day
                            </span>
                          </div>
                        </div>
                        <div
                          data-v-0113e2d8=""
                          className="ivu-row-flex margin-bottom-10"
                          data-v-4214013e=""
                        >
                          <div
                            data-v-0113e2d8=""
                            className="ivu-col"
                            data-v-4214013e=""
                            style={{ flex: "1 1 auto" }}
                          >
                            <div
                              data-v-4214013e=""
                              className="finance-card__img-box"
                            >
                              <img
                                data-v-4214013e=""
                                src="https://image.treasurenft.xyz/PC/img/img_pledge_01_lv04.png"
                                alt="finance-level-img"
                                height={207}
                                width={309}
                                className="finance-card__img"
                              />
                              <div
                                data-v-4214013e=""
                                className="finance-card__wedge"
                                style={{
                                  backgroundImage:
                                    'url("https://image.treasurenft.xyz/PC/img/img_wedge_02.svg")'
                                }}
                              >
                                <div
                                  data-v-0113e2d8=""
                                  className="ivu-row-flex ivu-row-flex-middle"
                                  data-v-4214013e=""
                                  style={{
                                    marginLeft: "-3.5px",
                                    marginRight: "-3.5px",
                                    height: "100%"
                                  }}
                                >
                                  <div
                                    data-v-0113e2d8=""
                                    className="ivu-col margin-left-45"
                                    data-v-4214013e=""
                                    style={{
                                      paddingLeft: "3.5px",
                                      paddingRight: "3.5px"
                                    }}
                                  >
                                    <img
                                      data-v-4214013e=""
                                      width={24}
                                      height={24}
                                      src="https://image.treasurenft.xyz/PC/img/icon_curve_up_01.svg"
                                      alt="curve up icon"
                                      className="finance-card__icon"
                                    />
                                  </div>
                                  <div
                                    data-v-0113e2d8=""
                                    className="ivu-col"
                                    data-v-4214013e=""
                                    style={{
                                      paddingLeft: "3.5px",
                                      paddingRight: "3.5px"
                                    }}
                                  >
                                    <span
                                      data-v-4214013e=""
                                      className="finance-card__wedge-text"
                                    >
                                      3%
                                    </span>
                                  </div>
                                </div>
                              </div>
                            </div>
                          </div>
                        </div>
                        <div
                          data-v-0113e2d8=""
                          className="ivu-row-flex ivu-row-flex-middle ivu-row-flex-space-between"
                          data-v-4214013e=""
                          style={{ marginLeft: "-8px", marginRight: "-8px" }}
                        >
                          <div
                            data-v-0113e2d8=""
                            className="ivu-col"
                            data-v-4214013e=""
                            style={{
                              flex: "1 1 auto",
                              paddingLeft: 8,
                              paddingRight: 8
                            }}
                          >
                            <div
                              data-v-0113e2d8=""
                              className="ivu-row margin-bottom-10"
                              data-v-4214013e=""
                            >
                              <div
                                data-v-0113e2d8=""
                                className="ivu-col"
                                data-v-4214013e=""
                              >
                                <h4
                                  data-v-4214013e=""
                                  className="finance-card__title"
                                >
                                  PledgePro
                                </h4>
                              </div>
                            </div>
                            <div
                              data-v-0113e2d8=""
                              className="ivu-row-flex ivu-row-flex-middle ivu-row-flex-space-between"
                              data-v-4214013e=""
                            >
                              <div
                                data-v-0113e2d8=""
                                className="ivu-col"
                                data-v-4214013e=""
                              >
                                <span
                                  data-v-4214013e=""
                                  className="finance-card__field"
                                >
                                  Amount
                                </span>
                              </div>
                              <div
                                data-v-0113e2d8=""
                                className="ivu-col"
                                data-v-4214013e=""
                              >
                                <div data-v-4214013e="" className="setMiddle">
                                  <img
                                    data-v-4214013e=""
                                    src="data:image/png;base64,"
                                    alt="coin-icon"
                                    width={20}
                                    height={20}
                                    className="margin-right-5"
                                  />
                                  <span
                                    data-v-4214013e=""
                                    className="finance-card__value color-green"
                                  >
                                    100-30000
                                  </span>
                                </div>
                              </div>
                            </div>
                            <div
                              data-v-0113e2d8=""
                              className="ivu-row-flex ivu-row-flex-middle ivu-row-flex-space-between"
                              data-v-4214013e=""
                            >
                              <div
                                data-v-0113e2d8=""
                                className="ivu-col"
                                data-v-4214013e=""
                              >
                                <span
                                  data-v-4214013e=""
                                  className="finance-card__field"
                                >
                                  Daily income
                                </span>
                              </div>
                              <div
                                data-v-0113e2d8=""
                                className="ivu-col"
                                data-v-4214013e=""
                              >
                                <span
                                  data-v-4214013e=""
                                  className="finance-card__value"
                                >
                                  3%
                                </span>
                              </div>
                            </div>
                            <div
                              data-v-0113e2d8=""
                              className="ivu-row-flex ivu-row-flex-middle ivu-row-flex-space-between"
                              data-v-4214013e=""
                            >
                              <div
                                data-v-0113e2d8=""
                                className="ivu-col"
                                data-v-4214013e=""
                              >
                                <span
                                  data-v-4214013e=""
                                  className="finance-card__field"
                                >
                                  Fees
                                </span>
                              </div>
                              <div
                                data-v-0113e2d8=""
                                className="ivu-col"
                                data-v-4214013e=""
                              >
                                <span
                                  data-v-4214013e=""
                                  className="finance-card__value"
                                >
                                  0%
                                </span>
                              </div>
                            </div>
                          </div>
                        </div>
                        <div
                          data-v-0113e2d8=""
                          className="ivu-row"
                          data-v-4214013e=""
                        >
                          <div
                            data-v-0113e2d8=""
                            className="ivu-col"
                            data-v-4214013e=""
                          >
                            <hr
                              data-v-4214013e=""
                              className="finance-card__divider"
                            />
                          </div>
                        </div>
                        <div
                          data-v-0113e2d8=""
                          className="ivu-row"
                          data-v-4214013e=""
                        >
                          <div
                            data-v-0113e2d8=""
                            className="ivu-col"
                            data-v-4214013e=""
                          >
                            {/**/}
                            <div
                              data-v-0113e2d8=""
                              className="ivu-row"
                              data-v-4214013e=""
                            >
                              <div
                                data-v-0113e2d8=""
                                className="ivu-col"
                                data-v-4214013e=""
                              >
                                <button
                                  data-v-0113e2d8=""
                                  type="button"
                                  className="ivu-btn ivu-btn-success ivu-btn-long"
                                  data-v-4214013e=""
                                  fdprocessedid="m2wlrc"
                                >
                                  {/**/} {/**/} <span>Details</span>
                                </button>
                              </div>
                            </div>
                          </div>
                        </div>
                      </div>
                    </div>
                    <div
                      data-v-0113e2d8=""
                      className="ivu-col card-gap-y ivu-col-span-xs-24 ivu-col-span-sm-12 ivu-col-span-lg-8 ivu-col-span-xxl-6"
                      style={{ paddingLeft: 6, paddingRight: 6 }}
                    >
                      <div data-v-4214013e="" className="finance-card">
                        <div
                          data-v-0113e2d8=""
                          className="ivu-row-flex ivu-row-flex-middle ivu-row-flex-space-between margin-bottom-10"
                          data-v-4214013e=""
                        >
                          <div
                            data-v-0113e2d8=""
                            className="ivu-col"
                            data-v-4214013e=""
                          >
                            <span
                              data-v-4214013e=""
                              className="finance-card__field"
                            >
                              Level：
                            </span>
                            <span
                              data-v-4214013e=""
                              className="finance-card__value color-green"
                            >
                              Lv1 ~ Lv6
                            </span>
                          </div>
                          <div
                            data-v-0113e2d8=""
                            className="ivu-col"
                            data-v-4214013e=""
                          >
                            <span
                              data-v-4214013e=""
                              className="finance-card__day"
                            >
                              120day
                            </span>
                          </div>
                        </div>
                        <div
                          data-v-0113e2d8=""
                          className="ivu-row-flex margin-bottom-10"
                          data-v-4214013e=""
                        >
                          <div
                            data-v-0113e2d8=""
                            className="ivu-col"
                            data-v-4214013e=""
                            style={{ flex: "1 1 auto" }}
                          >
                            <div
                              data-v-4214013e=""
                              className="finance-card__img-box"
                            >
                              <img
                                data-v-4214013e=""
                                src="https://image.treasurenft.xyz/PC/img/img_pledge_01_lv05.png"
                                alt="finance-level-img"
                                height={207}
                                width={309}
                                className="finance-card__img"
                              />
                              <div
                                data-v-4214013e=""
                                className="finance-card__wedge"
                                style={{
                                  backgroundImage:
                                    'url("https://image.treasurenft.xyz/PC/img/img_wedge_02.svg")'
                                }}
                              >
                                <div
                                  data-v-0113e2d8=""
                                  className="ivu-row-flex ivu-row-flex-middle"
                                  data-v-4214013e=""
                                  style={{
                                    marginLeft: "-3.5px",
                                    marginRight: "-3.5px",
                                    height: "100%"
                                  }}
                                >
                                  <div
                                    data-v-0113e2d8=""
                                    className="ivu-col margin-left-45"
                                    data-v-4214013e=""
                                    style={{
                                      paddingLeft: "3.5px",
                                      paddingRight: "3.5px"
                                    }}
                                  >
                                    <img
                                      data-v-4214013e=""
                                      width={24}
                                      height={24}
                                      src="https://image.treasurenft.xyz/PC/img/icon_curve_up_01.svg"
                                      alt="curve up icon"
                                      className="finance-card__icon"
                                    />
                                  </div>
                                  <div
                                    data-v-0113e2d8=""
                                    className="ivu-col"
                                    data-v-4214013e=""
                                    style={{
                                      paddingLeft: "3.5px",
                                      paddingRight: "3.5px"
                                    }}
                                  >
                                    <span
                                      data-v-4214013e=""
                                      className="finance-card__wedge-text"
                                    >
                                      3.5%
                                    </span>
                                  </div>
                                </div>
                              </div>
                            </div>
                          </div>
                        </div>
                        <div
                          data-v-0113e2d8=""
                          className="ivu-row-flex ivu-row-flex-middle ivu-row-flex-space-between"
                          data-v-4214013e=""
                          style={{ marginLeft: "-8px", marginRight: "-8px" }}
                        >
                          <div
                            data-v-0113e2d8=""
                            className="ivu-col"
                            data-v-4214013e=""
                            style={{
                              flex: "1 1 auto",
                              paddingLeft: 8,
                              paddingRight: 8
                            }}
                          >
                            <div
                              data-v-0113e2d8=""
                              className="ivu-row margin-bottom-10"
                              data-v-4214013e=""
                            >
                              <div
                                data-v-0113e2d8=""
                                className="ivu-col"
                                data-v-4214013e=""
                              >
                                <h4
                                  data-v-4214013e=""
                                  className="finance-card__title"
                                >
                                  NFTGemGain
                                </h4>
                              </div>
                            </div>
                            <div
                              data-v-0113e2d8=""
                              className="ivu-row-flex ivu-row-flex-middle ivu-row-flex-space-between"
                              data-v-4214013e=""
                            >
                              <div
                                data-v-0113e2d8=""
                                className="ivu-col"
                                data-v-4214013e=""
                              >
                                <span
                                  data-v-4214013e=""
                                  className="finance-card__field"
                                >
                                  Amount
                                </span>
                              </div>
                              <div
                                data-v-0113e2d8=""
                                className="ivu-col"
                                data-v-4214013e=""
                              >
                                <div data-v-4214013e="" className="setMiddle">
                                  <img
                                    data-v-4214013e=""
                                    src="data:image/png;base64,"
                                    alt="coin-icon"
                                    width={20}
                                    height={20}
                                    className="margin-right-5"
                                  />
                                  <span
                                    data-v-4214013e=""
                                    className="finance-card__value color-green"
                                  >
                                    100-50000
                                  </span>
                                </div>
                              </div>
                            </div>
                            <div
                              data-v-0113e2d8=""
                              className="ivu-row-flex ivu-row-flex-middle ivu-row-flex-space-between"
                              data-v-4214013e=""
                            >
                              <div
                                data-v-0113e2d8=""
                                className="ivu-col"
                                data-v-4214013e=""
                              >
                                <span
                                  data-v-4214013e=""
                                  className="finance-card__field"
                                >
                                  Daily income
                                </span>
                              </div>
                              <div
                                data-v-0113e2d8=""
                                className="ivu-col"
                                data-v-4214013e=""
                              >
                                <span
                                  data-v-4214013e=""
                                  className="finance-card__value"
                                >
                                  3.5%
                                </span>
                              </div>
                            </div>
                            <div
                              data-v-0113e2d8=""
                              className="ivu-row-flex ivu-row-flex-middle ivu-row-flex-space-between"
                              data-v-4214013e=""
                            >
                              <div
                                data-v-0113e2d8=""
                                className="ivu-col"
                                data-v-4214013e=""
                              >
                                <span
                                  data-v-4214013e=""
                                  className="finance-card__field"
                                >
                                  Fees
                                </span>
                              </div>
                              <div
                                data-v-0113e2d8=""
                                className="ivu-col"
                                data-v-4214013e=""
                              >
                                <span
                                  data-v-4214013e=""
                                  className="finance-card__value"
                                >
                                  0%
                                </span>
                              </div>
                            </div>
                          </div>
                        </div>
                        <div
                          data-v-0113e2d8=""
                          className="ivu-row"
                          data-v-4214013e=""
                        >
                          <div
                            data-v-0113e2d8=""
                            className="ivu-col"
                            data-v-4214013e=""
                          >
                            <hr
                              data-v-4214013e=""
                              className="finance-card__divider"
                            />
                          </div>
                        </div>
                        <div
                          data-v-0113e2d8=""
                          className="ivu-row"
                          data-v-4214013e=""
                        >
                          <div
                            data-v-0113e2d8=""
                            className="ivu-col"
                            data-v-4214013e=""
                          >
                            {/**/}
                            <div
                              data-v-0113e2d8=""
                              className="ivu-row"
                              data-v-4214013e=""
                            >
                              <div
                                data-v-0113e2d8=""
                                className="ivu-col"
                                data-v-4214013e=""
                              >
                                <button
                                  data-v-0113e2d8=""
                                  type="button"
                                  className="ivu-btn ivu-btn-success ivu-btn-long"
                                  data-v-4214013e=""
                                  fdprocessedid="xbkeb"
                                >
                                  {/**/} {/**/} <span>Details</span>
                                </button>
                              </div>
                            </div>
                          </div>
                        </div>
                      </div>
                    </div>
                    <div
                      data-v-0113e2d8=""
                      className="ivu-col card-gap-y ivu-col-span-xs-24 ivu-col-span-sm-12 ivu-col-span-lg-8 ivu-col-span-xxl-6"
                      style={{ paddingLeft: 6, paddingRight: 6 }}
                    >
                      <div data-v-4214013e="" className="finance-card">
                        <div
                          data-v-0113e2d8=""
                          className="ivu-row-flex ivu-row-flex-middle ivu-row-flex-space-between margin-bottom-10"
                          data-v-4214013e=""
                        >
                          <div
                            data-v-0113e2d8=""
                            className="ivu-col"
                            data-v-4214013e=""
                          >
                            <span
                              data-v-4214013e=""
                              className="finance-card__field"
                            >
                              Level：
                            </span>
                            <span
                              data-v-4214013e=""
                              className="finance-card__value color-green"
                            >
                              Lv1 ~ Lv6
                            </span>
                          </div>
                          <div
                            data-v-0113e2d8=""
                            className="ivu-col"
                            data-v-4214013e=""
                          >
                            <span
                              data-v-4214013e=""
                              className="finance-card__day"
                            >
                              150day
                            </span>
                          </div>
                        </div>
                        <div
                          data-v-0113e2d8=""
                          className="ivu-row-flex margin-bottom-10"
                          data-v-4214013e=""
                        >
                          <div
                            data-v-0113e2d8=""
                            className="ivu-col"
                            data-v-4214013e=""
                            style={{ flex: "1 1 auto" }}
                          >
                            <div
                              data-v-4214013e=""
                              className="finance-card__img-box"
                            >
                              <img
                                data-v-4214013e=""
                                src="https://image.treasurenft.xyz/PC/img/img_pledge_01_lv06.png"
                                alt="finance-level-img"
                                height={207}
                                width={309}
                                className="finance-card__img"
                              />
                              <div
                                data-v-4214013e=""
                                className="finance-card__wedge"
                                style={{
                                  backgroundImage:
                                    'url("https://image.treasurenft.xyz/PC/img/img_wedge_02.svg")'
                                }}
                              >
                                <div
                                  data-v-0113e2d8=""
                                  className="ivu-row-flex ivu-row-flex-middle"
                                  data-v-4214013e=""
                                  style={{
                                    marginLeft: "-3.5px",
                                    marginRight: "-3.5px",
                                    height: "100%"
                                  }}
                                >
                                  <div
                                    data-v-0113e2d8=""
                                    className="ivu-col margin-left-45"
                                    data-v-4214013e=""
                                    style={{
                                      paddingLeft: "3.5px",
                                      paddingRight: "3.5px"
                                    }}
                                  >
                                    <img
                                      data-v-4214013e=""
                                      width={24}
                                      height={24}
                                      src="https://image.treasurenft.xyz/PC/img/icon_curve_up_01.svg"
                                      alt="curve up icon"
                                      className="finance-card__icon"
                                    />
                                  </div>
                                  <div
                                    data-v-0113e2d8=""
                                    className="ivu-col"
                                    data-v-4214013e=""
                                    style={{
                                      paddingLeft: "3.5px",
                                      paddingRight: "3.5px"
                                    }}
                                  >
                                    <span
                                      data-v-4214013e=""
                                      className="finance-card__wedge-text"
                                    >
                                      4%
                                    </span>
                                  </div>
                                </div>
                              </div>
                            </div>
                          </div>
                        </div>
                        <div
                          data-v-0113e2d8=""
                          className="ivu-row-flex ivu-row-flex-middle ivu-row-flex-space-between"
                          data-v-4214013e=""
                          style={{ marginLeft: "-8px", marginRight: "-8px" }}
                        >
                          <div
                            data-v-0113e2d8=""
                            className="ivu-col"
                            data-v-4214013e=""
                            style={{
                              flex: "1 1 auto",
                              paddingLeft: 8,
                              paddingRight: 8
                            }}
                          >
                            <div
                              data-v-0113e2d8=""
                              className="ivu-row margin-bottom-10"
                              data-v-4214013e=""
                            >
                              <div
                                data-v-0113e2d8=""
                                className="ivu-col"
                                data-v-4214013e=""
                              >
                                <h4
                                  data-v-4214013e=""
                                  className="finance-card__title"
                                >
                                  ProsperNFT Hub
                                </h4>
                              </div>
                            </div>
                            <div
                              data-v-0113e2d8=""
                              className="ivu-row-flex ivu-row-flex-middle ivu-row-flex-space-between"
                              data-v-4214013e=""
                            >
                              <div
                                data-v-0113e2d8=""
                                className="ivu-col"
                                data-v-4214013e=""
                              >
                                <span
                                  data-v-4214013e=""
                                  className="finance-card__field"
                                >
                                  Amount
                                </span>
                              </div>
                              <div
                                data-v-0113e2d8=""
                                className="ivu-col"
                                data-v-4214013e=""
                              >
                                <div data-v-4214013e="" className="setMiddle">
                                  <img
                                    data-v-4214013e=""
                                    src="data:image/png;base64,"
                                    alt="coin-icon"
                                    width={20}
                                    height={20}
                                    className="margin-right-5"
                                  />
                                  <span
                                    data-v-4214013e=""
                                    className="finance-card__value color-green"
                                  >
                                    100-80000
                                  </span>
                                </div>
                              </div>
                            </div>
                            <div
                              data-v-0113e2d8=""
                              className="ivu-row-flex ivu-row-flex-middle ivu-row-flex-space-between"
                              data-v-4214013e=""
                            >
                              <div
                                data-v-0113e2d8=""
                                className="ivu-col"
                                data-v-4214013e=""
                              >
                                <span
                                  data-v-4214013e=""
                                  className="finance-card__field"
                                >
                                  Daily income
                                </span>
                              </div>
                              <div
                                data-v-0113e2d8=""
                                className="ivu-col"
                                data-v-4214013e=""
                              >
                                <span
                                  data-v-4214013e=""
                                  className="finance-card__value"
                                >
                                  4%
                                </span>
                              </div>
                            </div>
                            <div
                              data-v-0113e2d8=""
                              className="ivu-row-flex ivu-row-flex-middle ivu-row-flex-space-between"
                              data-v-4214013e=""
                            >
                              <div
                                data-v-0113e2d8=""
                                className="ivu-col"
                                data-v-4214013e=""
                              >
                                <span
                                  data-v-4214013e=""
                                  className="finance-card__field"
                                >
                                  Fees
                                </span>
                              </div>
                              <div
                                data-v-0113e2d8=""
                                className="ivu-col"
                                data-v-4214013e=""
                              >
                                <span
                                  data-v-4214013e=""
                                  className="finance-card__value"
                                >
                                  0%
                                </span>
                              </div>
                            </div>
                          </div>
                        </div>
                        <div
                          data-v-0113e2d8=""
                          className="ivu-row"
                          data-v-4214013e=""
                        >
                          <div
                            data-v-0113e2d8=""
                            className="ivu-col"
                            data-v-4214013e=""
                          >
                            <hr
                              data-v-4214013e=""
                              className="finance-card__divider"
                            />
                          </div>
                        </div>
                        <div
                          data-v-0113e2d8=""
                          className="ivu-row"
                          data-v-4214013e=""
                        >
                          <div
                            data-v-0113e2d8=""
                            className="ivu-col"
                            data-v-4214013e=""
                          >
                            {/**/}
                            <div
                              data-v-0113e2d8=""
                              className="ivu-row"
                              data-v-4214013e=""
                            >
                              <div
                                data-v-0113e2d8=""
                                className="ivu-col"
                                data-v-4214013e=""
                              >
                                <button
                                  data-v-0113e2d8=""
                                  type="button"
                                  className="ivu-btn ivu-btn-success ivu-btn-long"
                                  data-v-4214013e=""
                                  fdprocessedid="9adx3v"
                                >
                                  {/**/} {/**/} <span>Details</span>
                                </button>
                              </div>
                            </div>
                          </div>
                        </div>
                      </div>
                    </div>
                    <div
                      data-v-0113e2d8=""
                      className="ivu-col card-gap-y ivu-col-span-xs-24 ivu-col-span-sm-12 ivu-col-span-lg-8 ivu-col-span-xxl-6"
                      style={{ paddingLeft: 6, paddingRight: 6 }}
                    >
                      <div data-v-4214013e="" className="finance-card">
                        <div
                          data-v-0113e2d8=""
                          className="ivu-row-flex ivu-row-flex-middle ivu-row-flex-space-between margin-bottom-10"
                          data-v-4214013e=""
                        >
                          <div
                            data-v-0113e2d8=""
                            className="ivu-col"
                            data-v-4214013e=""
                          >
                            <span
                              data-v-4214013e=""
                              className="finance-card__field"
                            >
                              Level：
                            </span>
                            <span
                              data-v-4214013e=""
                              className="finance-card__value color-green"
                            >
                              Lv1 ~ Lv6
                            </span>
                          </div>
                          <div
                            data-v-0113e2d8=""
                            className="ivu-col"
                            data-v-4214013e=""
                          >
                            <span
                              data-v-4214013e=""
                              className="finance-card__day"
                            >
                              180day
                            </span>
                          </div>
                        </div>
                        <div
                          data-v-0113e2d8=""
                          className="ivu-row-flex margin-bottom-10"
                          data-v-4214013e=""
                        >
                          <div
                            data-v-0113e2d8=""
                            className="ivu-col"
                            data-v-4214013e=""
                            style={{ flex: "1 1 auto" }}
                          >
                            <div
                              data-v-4214013e=""
                              className="finance-card__img-box"
                            >
                              <img
                                data-v-4214013e=""
                                src="https://image.treasurenft.xyz/PC/img/img_pledge_01_lv07.png"
                                alt="finance-level-img"
                                height={207}
                                width={309}
                                className="finance-card__img"
                              />
                              <div
                                data-v-4214013e=""
                                className="finance-card__wedge"
                                style={{
                                  backgroundImage:
                                    'url("https://image.treasurenft.xyz/PC/img/img_wedge_02.svg")'
                                }}
                              >
                                <div
                                  data-v-0113e2d8=""
                                  className="ivu-row-flex ivu-row-flex-middle"
                                  data-v-4214013e=""
                                  style={{
                                    marginLeft: "-3.5px",
                                    marginRight: "-3.5px",
                                    height: "100%"
                                  }}
                                >
                                  <div
                                    data-v-0113e2d8=""
                                    className="ivu-col margin-left-45"
                                    data-v-4214013e=""
                                    style={{
                                      paddingLeft: "3.5px",
                                      paddingRight: "3.5px"
                                    }}
                                  >
                                    <img
                                      data-v-4214013e=""
                                      width={24}
                                      height={24}
                                      src="https://image.treasurenft.xyz/PC/img/icon_curve_up_01.svg"
                                      alt="curve up icon"
                                      className="finance-card__icon"
                                    />
                                  </div>
                                  <div
                                    data-v-0113e2d8=""
                                    className="ivu-col"
                                    data-v-4214013e=""
                                    style={{
                                      paddingLeft: "3.5px",
                                      paddingRight: "3.5px"
                                    }}
                                  >
                                    <span
                                      data-v-4214013e=""
                                      className="finance-card__wedge-text"
                                    >
                                      4.5%
                                    </span>
                                  </div>
                                </div>
                              </div>
                            </div>
                          </div>
                        </div>
                        <div
                          data-v-0113e2d8=""
                          className="ivu-row-flex ivu-row-flex-middle ivu-row-flex-space-between"
                          data-v-4214013e=""
                          style={{ marginLeft: "-8px", marginRight: "-8px" }}
                        >
                          <div
                            data-v-0113e2d8=""
                            className="ivu-col"
                            data-v-4214013e=""
                            style={{
                              flex: "1 1 auto",
                              paddingLeft: 8,
                              paddingRight: 8
                            }}
                          >
                            <div
                              data-v-0113e2d8=""
                              className="ivu-row margin-bottom-10"
                              data-v-4214013e=""
                            >
                              <div
                                data-v-0113e2d8=""
                                className="ivu-col"
                                data-v-4214013e=""
                              >
                                <h4
                                  data-v-4214013e=""
                                  className="finance-card__title"
                                >
                                  CryptoCanvas Yield
                                </h4>
                              </div>
                            </div>
                            <div
                              data-v-0113e2d8=""
                              className="ivu-row-flex ivu-row-flex-middle ivu-row-flex-space-between"
                              data-v-4214013e=""
                            >
                              <div
                                data-v-0113e2d8=""
                                className="ivu-col"
                                data-v-4214013e=""
                              >
                                <span
                                  data-v-4214013e=""
                                  className="finance-card__field"
                                >
                                  Amount
                                </span>
                              </div>
                              <div
                                data-v-0113e2d8=""
                                className="ivu-col"
                                data-v-4214013e=""
                              >
                                <div data-v-4214013e="" className="setMiddle">
                                  <img
                                    data-v-4214013e=""
                                    src="data:image/png;base64,"
                                    alt="coin-icon"
                                    width={20}
                                    height={20}
                                    className="margin-right-5"
                                  />
                                  <span
                                    data-v-4214013e=""
                                    className="finance-card__value color-green"
                                  >
                                    100-200000
                                  </span>
                                </div>
                              </div>
                            </div>
                            <div
                              data-v-0113e2d8=""
                              className="ivu-row-flex ivu-row-flex-middle ivu-row-flex-space-between"
                              data-v-4214013e=""
                            >
                              <div
                                data-v-0113e2d8=""
                                className="ivu-col"
                                data-v-4214013e=""
                              >
                                <span
                                  data-v-4214013e=""
                                  className="finance-card__field"
                                >
                                  Daily income
                                </span>
                              </div>
                              <div
                                data-v-0113e2d8=""
                                className="ivu-col"
                                data-v-4214013e=""
                              >
                                <span
                                  data-v-4214013e=""
                                  className="finance-card__value"
                                >
                                  4.5%
                                </span>
                              </div>
                            </div>
                            <div
                              data-v-0113e2d8=""
                              className="ivu-row-flex ivu-row-flex-middle ivu-row-flex-space-between"
                              data-v-4214013e=""
                            >
                              <div
                                data-v-0113e2d8=""
                                className="ivu-col"
                                data-v-4214013e=""
                              >
                                <span
                                  data-v-4214013e=""
                                  className="finance-card__field"
                                >
                                  Fees
                                </span>
                              </div>
                              <div
                                data-v-0113e2d8=""
                                className="ivu-col"
                                data-v-4214013e=""
                              >
                                <span
                                  data-v-4214013e=""
                                  className="finance-card__value"
                                >
                                  0%
                                </span>
                              </div>
                            </div>
                          </div>
                        </div>
                        <div
                          data-v-0113e2d8=""
                          className="ivu-row"
                          data-v-4214013e=""
                        >
                          <div
                            data-v-0113e2d8=""
                            className="ivu-col"
                            data-v-4214013e=""
                          >
                            <hr
                              data-v-4214013e=""
                              className="finance-card__divider"
                            />
                          </div>
                        </div>
                        <div
                          data-v-0113e2d8=""
                          className="ivu-row"
                          data-v-4214013e=""
                        >
                          <div
                            data-v-0113e2d8=""
                            className="ivu-col"
                            data-v-4214013e=""
                          >
                            {/**/}
                            <div
                              data-v-0113e2d8=""
                              className="ivu-row"
                              data-v-4214013e=""
                            >
                              <div
                                data-v-0113e2d8=""
                                className="ivu-col"
                                data-v-4214013e=""
                              >
                                <button
                                  data-v-0113e2d8=""
                                  type="button"
                                  className="ivu-btn ivu-btn-success ivu-btn-long"
                                  data-v-4214013e=""
                                  fdprocessedid="1qn4fb"
                                >
                                  {/**/} {/**/} <span>Details</span>
                                </button>
                              </div>
                            </div>
                          </div>
                        </div>
                      </div>
                    </div>
                  </div>
                  {/**/}
                </div>




                <div
                  data-v-0113e2d8=""
                  className="ivu-tabs-tabpane"
                  
 style={{
  display: tabnavs === 'record' ? 'block' : 'none',
  visibility: tabnavs === 'record' ? 'visible' : 'hidden'
}}
                >
                  {/**/}
                  <div data-v-0113e2d8="" className="noData">
                    <img
                      data-v-0113e2d8=""
                      src="https://image.treasurenft.xyz/icon/icon_noData_01.svg"
                      alt="NoData"
                      loading="lazy"
                    />
                    <p
                      data-v-0113e2d8=""
                      className="title-black-PSB-16 font-weight-700 margin-top-30"
                    >
                      No Data Available
                    </p>
                  </div>
                </div>
              </div>
              <div
                className="ivu-tabs-context-menu"
                style={{ top: 0, left: 0 }}
              >
                <div className="ivu-dropdown">
                  <div className="ivu-dropdown-rel" /> {/**/}
                </div>
              </div>
            </div>
          </div>
          {/**/}
        </div>
        <img
          data-v-b02889ae=""
          src="https://image.treasurenft.xyz/btn/btn_help_01.png"
          alt="helpIcon"
          loading="lazy"
          className="helpIconStyle"
          style={{ display: "none" }}
        />
      </div>
    </div>
    {/**/}
    <div data-v-d27ce888="" data-v-b02889ae="">
      {/**/}
    </div>
    {/**/}
    {/**/}
    {/**/}
    {/**/}
    {/**/}
    <div data-v-d9d51de4="" data-v-b02889ae="">
      {/**/}
      {/**/}
    </div>
    {/**/}
    {/**/}
    {/**/}
  </div>
</div>
 
   
    
    </>
  );
};

export default Collection; 